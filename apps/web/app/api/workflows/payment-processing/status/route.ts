/**
 * Payment Processing Workflow Status API Route
 * 
 * Check the status and get results of payment processing workflows.
 * Example: GET /api/workflows/payment-processing/status?workflowId=payment-processing-123-4567890
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('🔍 Checking payment processing workflow status:', workflowId);
    ...
    if (isRunning) {
      return NextResponse.json({
        ...
        status: 'running',
        result: null,
        message: 'Payment processing workflow is still running',
      });
    }
    ...
    try {
      ...
      console.error('Failed to get payment processing workflow result:', resultError);
      ...
      return NextResponse.json({
        ...
        status: 'failed',
        result: null,
        error: 'Payment processing workflow failed or not found',
        message: resultError instanceof Error ? resultError.message : 'Unknown error',
      }, { status: 404 });
    }

  } catch (error) {
    console.error('Failed to check payment processing workflow status:', error);
    
    return NextResponse.json({
      success: false,
      error: 'Failed to check payment processing workflow status',
